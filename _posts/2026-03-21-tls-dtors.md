...
Now that we've overwritten the necessary fields, we can trigger `exit()` by breakpoint out of the program or giving an invalid value to `scanf`

![rip](rip_control.png)

Looking at the registers we also see `rdi` pointers to our string `/bin/sh`
