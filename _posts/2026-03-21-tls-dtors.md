...
For our purposes, the important takeaway is not the exact generation routine, but the fact that the pointer guard is initialized to a per-process randomized value.
 
As a result, forging a valid mangled function pointer requires either recovering the guard or overwriting it.
